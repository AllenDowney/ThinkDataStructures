...
proportional to \java{n} for all loops, you can get away with just
counting the loops. But if, as in this example, the number of
iterations is not always proportional to \java{n}, you have to give it
more thought.
...
$O(1)$ or constant time and \java{n} means $O(n)$ or
linear.
...
Where $\approx$ is the symbol for ``approximately equal''. Now, if we take the
...
In the subdirectory \java{javacs-lab05/src/com/allendowney/javacs}
you'll find the source files you need for this exercise:
...
\section{Filling in \java{WikiPhilosophy}}
\label{filling-in-wikiphilosophy}
...
In the subdirectory \java{javacs-lab06/src/com/allendowney/javacs}
you'll find the source files for this exercise:

\begin{verbatim}
*  \java{TermCounter.java} contains the code from the previous section.

*  \java{Index.java} contains the class definition for the next part of this exercise.

*  \java{WikiFetcher.java} contains the class we used in the previous exercise to download and parse Web pages.

*  \java{WikiNodeIterable.java} contains the class we used to traverse the nodes in a DOM tree.
\end{verbatim}
...
In the subdirectory \java{javacs-lab07/src/com/allendowney/javacs}
you'll find the source files for this exercise:

\begin{verbatim}
*  \java{MyLinearMap.java} contains starter code for the first part of the exercise.

*  \java{MyLinearMapTest.java} contains the unit tests for \java{MyLinearMap}.
\end{verbatim}
...
the speedup when we have $k$ maps might be much less than
$k$. So one of the goals of a hash function is to be uniform;
...
In the subdirectory \java{javacs-lab08/src/com/allendowney/javacs}
you'll find the source files for this exercise:
...
size of \java{n/k}. For example, suppose we double $k$ every
...
In the subdirectory \java{javacs-lab09/src/com/allendowney/javacs}
...
\java{h}, not $n$. Ideally you should search the tree only
...
\java{log n}, which is much less than $n$. In fact,
\java{log n} grows so slowly as $n$ increases, it can be
...
In the subdirectory \java{javacs-lab10/src/com/allendowney/javacs}
you'll find the source files for this exercise:
...
In the subdirectory \java{javacs-lab11/src/com/allendowney/javacs}
you'll find the source files for this exercise:

\begin{verbatim}
*  \java{WikiCrawler.java}, which contains starter code for your crawler.

*  \java{WikiCrawlerTest.java}, which contains test code for \java{WikiCrawler}.

*  \java{JedisIndex.java}, which is our solution to the previous exercise.
\end{verbatim}

You'll also find some of the helper classes we've used in previous lavs

\begin{verbatim}
*  \java{JedisMaker.java}
*  \java{WikiFetcher.java}
*  \java{TermCounter.java}
*  \java{WikiNodeIterable.java}
\end{verbatim}

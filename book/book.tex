...
available at \url{https://thinkdast.com/cc30}.
...
Permission is granted to copy, distribute, and/or modify this work under the terms of the Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License, which is available at \url{https://thinkdast.com/cc30}.
...
%TODO: Reorganize this chapter (see comments from Patrick White)
...
\url{https://thinkdast.com/getphil}).
...
%TODO: move this section to preface?
...
\url{https://thinkdast.com/interface}.
...
I'll start by reviewing \java{interface}s, and the classes that implement
them, and I'll present the idea of ``programming to an
interface''.
...
\url{https://thinkdast.com/ternary}.


\section{\java{interface List}}
...
(see \url{https://thinkdast.com/interbaseprog}).
...
\begin{itemize}
  \item
    \texttt{build.xml} is an Ant file that makes it easier to compile
    and run the code.

  \item
    \texttt{lib} contains the libraries you'll need (for this
    exercise, just JUnit).
...
And if you navigate into \texttt{src/com/allendowney/thinkdast},
...
{\bf selection sort}
(see \url{https://thinkdast.com/selectsort}):
...
the first, we can compute the location any other element
...
have to instantiate an array of \java{Object} and then typecast it. You can
read more about this issue at
\url{https://thinkdast.com/generics}.
...
\url{https://thinkdast.com/colladd}.
...
\url{https://thinkdast.com/listset}, then fill in the body of this method. If you run
...
\url{https://thinkdast.com/listindof}
so you know what it's supposed to do. In particular, notice how it is
...
\url{https://thinkdast.com/listadd},
...
\url{https://thinkdast.com/listrem}. When
you finish this one, all tests should pass.

Once you have your implementation working, compare it to mine, which you
can read at
\url{https://thinkdast.com/myarraylist}.
...
% TODO: Maybe explain why resizing is linear?
...
\url{https://thinkdast.com/linkedlist},
...
Integers 0, 1, 2, and 3 as data, linked up in increasing order. In the
last node, the \java{next} field is
\java{null}.

Figure~\ref{linkedlistfig} shows these variables and the objects they
refer to.
...
Again, read the documentation at
\url{https://thinkdast.com/listadd}, write an implementation, and run the tests for
confirmation.

Last one: fill in the body of \java{remove}.  The documentation is
here:
\url{https://thinkdast.com/listrem}.
...
This chapter presents solutions to the
...
\url{https://thinkdast.com/anonclass}.
...
exercise. When we ran it, we got this output:
...
%TODO: help for people who are shaky about logs?
...
\url{https://thinkdast.com/linked},
it says:
...
about them at \url{https://thinkdast.com/doublelist},
...
A \textbf{Web search engine}, like Google Search
or Bing, takes a set of ``search terms'' and returns a list of web
pages that are relevant to those terms (we'll discuss what ``relevant''
means later). 
You can read more at 
\url{https://thinkdast.com/searcheng},
...
about at 
\url{https://thinkdast.com/jsoupelt},
\url{https://thinkdast.com/jsoupelts}, and
\url{https://thinkdast.com/jsoupnode}.

% TODO: Provide more information about these classes here, rather than
% relying on the links?
...
%\label{fig-uml2}
% Edit: https://yuml.me/edit/4bc1c919
%\end{figure}

%Figure~\ref{fig-uml2} shows the relationships among these classes.
%In a UML class diagram, a line with a hollow arrow head indicates
%that one class extends another.  For example, this diagram
%indicates that \java{Elements} extends \java{ArrayList}.
...
\url{https://thinkdast.com/iterator}
and
\url{https://thinkdast.com/iterable}.
...
\url{https://thinkdast.com/singleton}.
...
\item
  ...
  (\url{https://thinkdast.com/philosophy} after following
...
\item
  ...
  you can read more about them at \url{https://thinkdast.com/regex}.
...
\item  \java{TermCounterTest.java} contains test code for \java{TermCounter.java}.
...
from search terms to counts. 

% TODO: Draw a diagram (both reviewers)!
...
\java{Map.put} at
  \url{https://thinkdast.com/listput}
...
% TODO: rewrite this to handle negative hashcodes, and modify the
% following paragraph
...
then apply the remainder operator, \java{\%}, which guarantees that the
...
\url{https://thinkdast.com/codepoint}.
...
\url{https://thinkdast.com/hash}.


\section{Hashing and mutation}
\label{hashing-and-mutation}

Strings are immutable, and \java{SillyString} is also immutable
...
to a different \java{String}, and you can change the \java{String} it
...
$n/k$, and $k$ grows in proportion to $n$,
several of the core \java{MyBetterMap} methods
...
It also presents the binary search tree, which is an efficient
...
Language
(see \url{https://thinkdast.com/uml}).
...
\label{fig-uml}
% Edit: https://yuml.me/edit/2aa18a2d
\end{figure}
...
% TODO: Add info about computing good hashes?
...
\url{https://thinkdast.com/bst}, which you
...
% TODO: add IllegalArgumentException to the code
...
\url{https://thinkdast.com/gentut}.
...
\url{https://thinkdast.com/mapkeyset},
this method should return a \java{Set} that iterates the keys in
order; that is, in increasing order according to the \java{compareTo}
method.  The \java{HashSet} implementation of \java{Set}, which we
used in Section~\ref{exercise6}, doesn't maintain the order of the
keys, but the \java{LinkedHashSet} implementation does.  You can read
about it at
\url{https://thinkdast.com/linkedhashset}.
...
\url{https://thinkdast.com/inorder}.
...
\url{https://thinkdast.com/balancing}.
...
\url{https://thinkdast.com/bstdel}.
...
using Java maps. In this exercise, we'll revisit the \java{Indexer} and make
...
like JSON (\url{https://thinkdast.com/json}) and then write them
...
\url{https://thinkdast.com/database}.
...
\item
  ...
  service like RedisToGo (\url{https://thinkdast.com/redistogo}), which runs
...
\item Create an account on RedisToGo, at
  \url{https://thinkdast.com/redissign},
...
Redis is basically a map from keys, which are \java{String}s, to
values, which can be one of several data types. The most basic Redis
data type is a \java{String}. To add a \java{String} to the database,
...
Redis provides a Set structure, which is
similar to a Java
\java{Set<String>}. To add elements to a Redis Set,
you choose a key to identify the set and then use
\java{jedis.sadd}:
...
Redis also provides a list structure, which is
similar to a Java
...
The keys and values are strings, so if we want to store
an \java{Integer}, so we have to convert it to
...
\url{https://thinkdast.com/redistypes}.
...
Hashes at \url{https://thinkdast.com/redistypes}

In \java{JedisIndex}, I provide a function that takes a search term
...
context as stop words (\url{https://thinkdast.com/stopword}).
...
them at \url{https://thinkdast.com/graph}.
...
% TODO: consider using ``buffer'' instead of ``queue''
...
\url{https://thinkdast.com/graphtrav}.
...
Section~\ref{redis-indexer}. So the only new function is
\java{queueInternalLinks}.

% TODO: should we add comments to methods and identify their classes?

I wrote two versions of this function with different parameters: one
...
% TODO: make the indentation of methods consistent?
...
\url{https://thinkdast.com/infret}.
...
There are two version of \java{sort}:
...
as required. Then it creates an instance of the newly-defined,
unnamed class. If you are not familiar with anonymous classes in Java, you can
read about them at
\url{https://thinkdast.com/anonclass}.
...
\item Read about TF-IDF at \url{https://thinkdast.com/tfidf}
...
\item Build a user interface that allows users to enter queries with
  ...
  \url{https://thinkdast.com/heroku}.
...
\url{https://thinkdast.com/insertsort}, which includes
...
The following example shows how to call this function with a \java{List} of
\java{Integer} objects:

% TODO: in the code, change n and m to elt1 and elt2
...
quadratic. Again, rather than
explaining the algorithm here, I suggest you
read about it on
Wikipedia at \url{https://thinkdast.com/mergesort}.
Once you get the idea, come back and you can test your
understanding by writing an implementation.
...
\url{https://thinkdast.com/basecase}. If you are
...
See \url{https://thinkdast.com/compsort}.
...
See \url{https://thinkdast.com/bubble}.
...
\url{https://thinkdast.com/radix}. As an optional
...
\url{https://thinkdast.com/extsort}.
...
%TODO: also mention that the heap is completely balanced, which
% makes it easy to implement with an array
...
(see \url{https://thinkdast.com/heapsort}).
...
\item If a dataset doesn't fit into the memory of one program, the runtime
  ...
  CPU caches and run
  faster (see \url{https://thinkdast.com/cache}.

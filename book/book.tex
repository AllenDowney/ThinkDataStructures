...
completion.  You can find more information at \url{URL_coming_soon}.
...
like list \java{List}.  It should not depend on a specific
...
\item In general, the number of comparisons is $n$ - \java{start}.
...
\url{https://docs.oracle.com/javase/8/docs/api/java/util/List.html\#set(int,\%20E)}, then fill in the body of this method. If you run
...
\url{https://docs.oracle.com/javase/8/docs/api/java/util/List.html\#indexOf(java.lang.Object)}{read
...
Again,
\url{https://docs.oracle.com/javase/8/docs/api/java/util/List.html\#add(int,\%20E)}{read
...
\url{https://docs.oracle.com/javase/8/docs/api/java/util/List.html\#remove(int)}{The
...
To make things a little easier, I provide a helper method called
\java{equals} that compares an element from the array to a target
...
\url{https://docs.oracle.com/javase/8/docs/api/java/util/List.html\#add(int,\%20E)}, write an implementation, and run the tests for
confirmation.
...
\item
  \java{ProfileListAdd.java} contains starter code for this exercise, including
...
The next exercise asks you to profile the performance of adding new
...
% NOTE: This is the only mention of synchronization.  Remove or expand?
...
It might not be obvious that it is worthwhile to rewrite
a perfectly good method
method with two classes and five methods.  But now that we've done
it, we can use \java{WikiNodeIterable} anywhere an \java{Iterable}
is called for, which make it easy an syntactially clean to separate
the logic of the iteration (DFS) from whatever processing we are
doing on the nodes.
...
exercises, we'll look at these implementations and analyze their performance.
...
This exercise involves more reading than some of the others, and not
as much programming. But I hope you learned a lot. Congratulations on
getting to the end!
...
In the next lesson, I'll present a solution, analyze the performance
...
% TODO: rewrite this to handle negative hashcodes
...
\begin{figure}
\centerline{\includegraphics[width=5in]{figs/towers.pdf}}
...
Language
(\url{https://en.wikipedia.org/wiki/Unified_Modeling_Language}; a
class diagram is one of several graphical standards defined by UML.
...
\url{https://docs.oracle.com/javase/8/docs/api/java/util/TreeMap.html\#keySet()}, this method should return some kind of

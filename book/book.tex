...
If you navigate into \texttt{src/com/allendowney/thinkdast},
...
to those terms (we'll discuss what ``relevant'' means later).  You can
read more at \url{http://thinkdast.com/searcheng}, but we'll explain
...
\java{Elements} is a collection of \java{Element} objects.
...
\item
  ...
  (\url{http://thinkdast.com/philosophy} after following
...
there. Notice that we have provided an \java{equals} method that
...
them use \java{findNode}, which is a private method we defined; it is
...
we've provided a helper method, \java{equals}, that compares
...
The runtime is more than 7 times longer than in the previous case.
...
Here's our implementation of \java{WikiCrawler.crawl}:

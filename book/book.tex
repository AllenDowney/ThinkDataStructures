...
\item If you don't want to use Git at all, you can download the code
  ...
  page, or this link:
  \url{https://github.com/AllenDowney/ThinkDataStructures/archive/master.zip}.

% or http://tinyurl.com/thinkdastcode

% TODO: make this short link
...
\java{List} \java{interface}?  And how should you choose which one to
...
\java{collection}. If \java{collection} contains m elements and the list we are
...
If you open \java{JedisMaker.java}, you should see the java{JedisMaker}
class, which is a helper class that provides one static method,
\java{make}, which creates a \java{Jedis} object. Once this object
is authenticated, you can use it to
...
we want, like links to pages in non-English languages. But let's keep it
simple.
...
familiar with anonymous classes in Java, you can read about them at
\url{https://docs.oracle.com/javase/tutorial/java/javaOO/anonymousclasses.html}.
...
At the top level, we have $1$ list with $n$ elements. At
the next level there are $2$ lists with $n/2$ elements.
...
\item
  How many times do we have to double $1$ before we get to
  $n$.
